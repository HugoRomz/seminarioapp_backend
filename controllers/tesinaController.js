...
const createInvitation = async (req, res) => {
    try {
        ...
        if (!Array.isArray(invitado_email) || invitado_email.length === 0) {
            return res.status(400).json({ error: 'Debe proporcionar al menos un correo electrónico.' });
        }
        ...
        for (const email of invitado_email) {
            const invitado = await Usuarios.findOne({ where: { email_usuario: email } });
            ...
            const invitadoSentInvitation = await Invitaciones.findOne({ where: { usuario_id: invitado.usuario_id } });
            if (invitadoSentInvitation) {
                pendingInvitations.push(email);
                continue;
            }

            const pendingInvitation = await Invitaciones.findOne({ where: { usuario_id_invitado: invitado.usuario_id } });
            ...
            await sendEmailInvitation(email, req.user.nombre, nombre_tesina, area_tema, resenia_tema);
        }
...
const createTesina = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Tesina registrada con éxito.', tesina: nuevaTesina });
    } catch (error) {
...
const getTesinasByUser = async (req, res) => {
    try {
        ...
        if (tesinas.length === 0) {
            return res.status(404).json({ message: 'No se encontraron tesinas para este usuario.' });
        }

        res.status(200).json(tesinas);
    } catch (error) {
        console.error('Error al obtener las tesinas del usuario:', error);
        res.status(500).json({ message: 'Error al obtener las tesinas del usuario.', error });
    }
};
